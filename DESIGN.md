...
### Skin Cosmetic Effects
Princesa/Vôlei/Noiva periodically emit a small `Path2D` star or heart sparkle (same generic `Particles.sparkle` emitter, different color/shape/interval per skin) with a `2.2px` ink outline — the same flat-ink-on-paper treatment as every other drawn glyph in the game, added specifically because pale fills (the bridal white originally chosen for Noiva) are otherwise invisible against the equally pale sky. Gold gets a continuous soft golden radial-gradient glow: no ink outline, since a gradient glow is meant to look like light, not a bordered object. Pijama shows a small "Z z z" (Body face) above Mariana's head only while she's idle on the main menu, not during a run.
