...
- One vocabulary everywhere: entrances fade + rise 8–22px + a quick 6px blur that clears in the first half (`cubic-bezier(0.16,1,0.3,1)`), staggered 55–70ms; hover/press/toasts use `cubic-bezier(0.22,1,0.36,1)`; state swaps use `cubic-bezier(0.4,0,0.2,1)`. Entrances animate the independent `translate`/`scale` properties with `backwards` fill so `transform` stays free for hover/press.
- Hover lift (translateY -2px, scale 1.02, print-shadow grows) only under `(hover: hover) and (pointer: fine)`; presses respond in 60ms everywhere.
- The portal loading overlay (`#overlay-transition`) and the biome system are explicitly out of scope for this layer — no rule targets `.overlay`, `.title` or `.title--small` generically.
- `prefers-reduced-motion`: every entrance collapses to a 140ms fade; no parallax, blur, loops, count-up or camera moves.

### Guarda-Roupa (Wardrobe) Screen
- **Style (2026-09-25):** full-bleed paper overlay (`z-index: 3`) laid out as header (title, Mariana/Alberto segmented tabs, coin chip) + a carousel stage + an info panel. Landscape puts stage and info side by side; portrait stacks them. The stage uses `container-type: size`, so item sizes come from the stage's real box (cqw/cqh) instead of a global scale.
- **Carousel:** role-based, TOONHUB-style — the active skin is centered, full size and sharp; the previous/next shrink to the sides with a slight blur and opacity drop; two more sit faintly behind; the rest are hidden off-stage. Navigating reassigns roles and CSS animates transform/opacity/filter together (650ms, `cubic-bezier(0.4,0,0.2,1)`), with a JS lock blocking new navigation until the swap ends. Depth comes from the flat arch + ink-bordered floor disc around the skin, never from filters on the sprite (the center item is always unfiltered). Arrows, swipe, tapping a side item, and ←/→ keys all navigate; Esc closes.
- **Info + action:** skin glyph, name, price (or GRÁTIS / NO SEU GUARDA-ROUPA), EQUIPADA tag, and one action button: COMPRAR (`--accent`), EQUIPAR (ink), EQUIPADA or locked COMPRAR (paper-dim, `aria-disabled`). A locked tap shakes the button and shows "Faltam N moedas" inline. A purchase pops the button, floats "-N" under the coin chip, hops the preview and stamps DESBLOQUEADA!. Purchase/equip rules are unchanged (buying never auto-equips).
